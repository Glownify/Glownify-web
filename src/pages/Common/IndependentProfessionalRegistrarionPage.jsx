...
const IndependentProfessionalRegisterPage = () => {
  ...
  const btnPrimary = "flex items-center justify-center gap-2 bg-pink-600 hover:bg-pink-700 text-white font-bold py-2.5 sm:py-3 px-6 sm:px-8 rounded-xl transition-all shadow-lg shadow-pink-200 active:scale-95 text-sm sm:text-base";
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className='flex flex-col items-center mb-6 sm:mb-10'>
              <div className="relative group">
                <label htmlFor='userimg' className='bg-pink-50 h-24 w-24 sm:h-32 sm:w-32 flex items-center justify-center rounded-2xl sm:rounded-3xl cursor-pointer border-2 border-dashed border-pink-200 group-hover:border-pink-500 transition-all overflow-hidden'>
                  <div className="text-center">
                    <ImagePlus className='text-pink-400 mx-auto' size={24} />
              ...
              <h2 className="text-lg sm:text-xl font-bold text-gray-800 mt-4 sm:mt-6">Basic Information</h2>
        ...
        {step === 2 && (
          <div className="animate-in fade-in slide-in-from-right-4 duration-500">
            <header className="mb-6 sm:mb-8">
              <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Set Availability</h2>
                ...
                <div className="flex flex-wrap gap-3">
                  {daysList.map(day => (
                    <button type='button' key={day} onClick={() => toggleSelection(day, selectedDays, setSelectedDays)}
                      className={`h-10 w-14 sm:h-12 sm:w-16 flex items-center justify-center font-bold text-sm rounded-xl sm:rounded-2xl border-2 transition-all ${selectedDays.includes(day) ? "bg-pink-600 border-pink-600 text-white shadow-md shadow-pink-200" : "border-gray-100 text-gray-400 hover:border-pink-200"}`}>{day}</button>
                  ))}
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {timeSlotsList.map(slot => (
                    <button type='button' key={slot} onClick={() => toggleSelection(slot, selectedSlots, setSelectedSlots)}
                      className={`py-3 sm:py-4 px-2 border-2 rounded-xl sm:rounded-2xl font-bold text-sm transition-all ${selectedSlots.includes(slot) ? "bg-pink-50 border-pink-500 text-pink-600" : "border-gray-100 text-gray-400 hover:bg-gray-50"}`}>
                      {slot}
                    </button>
                  ))}
                </div>
        ...
        {step === 3 && (
          <div className="animate-in fade-in zoom-in-95 duration-500 max-w-md mx-auto">
            <header className="text-center mb-10">
              <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Verification</h2>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-1">
                ...
                <label htmlFor="docs" className="group border-2 border-dashed border-gray-200 p-5 sm:p-8 flex flex-col items-center rounded-2xl cursor-pointer hover:border-pink-500 hover:bg-pink-50/30 transition-all">
