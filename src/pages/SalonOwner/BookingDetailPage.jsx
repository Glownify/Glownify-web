...
const BookingDetailPage = () => {
    ...
    if (!booking) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center space-y-4">
                <p className="text-xl font-bold text-slate-800">Booking information not found</p>
                <button
                    onClick={() => navigate(-1)}
                    className="px-6 py-2 bg-pink-500 text-white rounded-xl font-bold"
                >
                    Go Back
                </button>
            </div>
    ...
    const avatar = customer.avatar || `https://i.pravatar.cc/150?u=${booking._id}`;
                ...
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        ...
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                            Appointment ID: <span className="text-pink-500">#{booking._id?.slice(-6).toUpperCase() || 'N/A'}</span>
                        </p>
                    </div>
                </div>
                    ...
                    <div className="bg-white rounded-[3.5rem] p-12 border border-slate-100 shadow-sm relative overflow-hidden">
                        ...
                        <div className="relative flex flex-col md:flex-row items-center md:items-start gap-10">
                            ...
                            <div className="flex-1 space-y-6 text-center md:text-left pt-4">
                                <div>
                                    <h2 className="text-5xl font-black text-slate-800 tracking-tight">{customer.name}</h2>
                                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 mt-4">
                                        <div className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${status === 'completed' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
                                            {status}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-6 pt-6 border-t border-slate-100">
                                    <QuickInfo icon={Phone} label="Call Now" value={customer.phoneNumber || "Verified"} color="text-teal-500" />
                                    <QuickInfo icon={MessageSquare} label="Message" value="Open Chat" color="text-[#8B5CF6]" />
                                    <QuickInfo icon={Mail} label="Email" value={customer.email?.slice(0, 10) + "..." || "Not Shared"} color="text-blue-500" />
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                            <div className="bg-slate-50/50 p-8 rounded-[2.5rem] border border-slate-100">
                                ...
                                <div className="mt-6 space-y-2">
                                    <h4 className="text-xl font-black text-slate-800">{dateFormatted}</h4>
                                    <p className="text-sm font-bold text-slate-400 capitalize">{formatTime(booking.timeSlot?.start)} – {formatTime(booking.timeSlot?.end)} <span className="mx-2 text-slate-200">|</span> {booking.bookingType === 'in_salon' ? 'Salon Visit' : 'Home Service'}</p>
                                </div>
                            </div>
                            ...
                            <div className="bg-slate-50/50 p-8 rounded-[2.5rem] border border-slate-100">
                                ...
                                <div className="mt-6 flex items-center gap-4">
                                    <img src={specialist.avatar || `https://i.pravatar.cc/150?u=${specialist._id}`} className="w-14 h-14 rounded-2xl object-cover shadow-sm" alt={specialist.name} />
                                    <div>
                                        <h4 className="text-lg font-black text-slate-800">{specialist.name}</h4>
                                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{specialist.role || "Professional"}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div className="xl:col-span-4 space-y-8">
                    ...
                    <div className="bg-white border border-slate-100 rounded-[3rem] p-10">
                        ...
                        <div className="space-y-6">
                            ...
                            <PolicyRow icon={Navigation} text={booking.serviceLocation?.address || "Address Hidden"} />
                        </div>
                    </div>
