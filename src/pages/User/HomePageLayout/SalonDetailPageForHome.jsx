...
const SalonDetailPageForHome = () => { 
    const dispatch = useDispatch();
    const {id}=useParams(); 
    
    const {saloonDetails,loading,error}=useSelector((state)=>state.user);


    useEffect(() => {
        // const salonId = localStorage.getItem("salonIdForHomeDetails");
        if (id) {
      ...
      if (loading) return <p className="p-4">Loading  home salon details...</p>;
  if (error) return <p className="p-4 text-red-500"> {error.message || "Something went wrong"}</p>;
...
export default SalonDetailPageForHome