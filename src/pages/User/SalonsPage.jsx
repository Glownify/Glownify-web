...
const SalonsPage = () => {
  ...
  const categoryFromUrl = searchParams.get("category") || "unisex";
  ...
  useEffect(() => {
    const loadLocation = async () => {
      ...
      setLocationReady(true); // ✅ ab safe hai
    };
  ...
  const fetchSalons = async (pageNumber = 1) => {
    try {
      ...
      await dispatch(fetchNearbySalons(params)).unwrap();
    } catch (err) {
      console.error(err);
      setFetchError(true);
  ...
  }, [page, hasMore, salonsLoading]);


  const rawSalons = nearbySalons || [];
  ...
  // Show skeleton only on the very first load before any fetch completes
  // const isLoading = salonsLoading ;
  const isLoading = salonsLoading || !locationReady;
  ...
  return (
    <div className="min-h-screen bg-[#FFF8F1] py-12 px-4">
      <div className="w-full mx-auto px-4 md:px-8 lg:px-12">
        ...
        <div className="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">
              Nearby{" "}
              <span className="text-indigo-600 capitalize">
                {selectedCategory === "unisex" ? "Salons" : `${selectedCategory}s`}
              </span>
            </h1>
            <p className="mt-2 text-slate-500 font-medium">
              {isLoading
        ...
        {salonsLoading && (
          <p className="text-center text-gray-400 mt-6">Loading more salons...</p>
        )}
        ...
        {/* ── Hard Error State (API failed AND no dummy rendered — extremely unlikely) ── */}
        {!isLoading && displaySalons.length === 0 && !nameFilter && (
          <div className="flex flex-col items-center justify-center py-24 text-center gap-5">
            <div className="w-20 h-20 bg-rose-50 rounded-full flex items-center justify-center">
              <Frown size={40} className="text-rose-400" />
            </div>
